...
export default function AdminTareasPage() {
  ...
  const load = useCallback(() => {
    ...
    ticketsApi
      ...
      .catch((e: any) => setError(e.message ?? tt("admintasks.loadError")))
  ...
  return (
    <ITPage
      ...
      description={tt("admintasks.description", { count: rows.length })}
      backAction={() => navigate(-1)}
      icon={<FaTasks size={20} />}
      breadcrumbs={[{ label: tt("common:breadcrumbs.home"), onClick: () => navigate("/") }, { label: tt("admintasks.breadcrumb") }]}
      actions={
        <ITFlex gap={2}>
          <ITButton variant="outlined" color="secondary" onClick={() => navigate("/tickets/kanban")}>
            <ITFlex align="center" gap={1}><FaTrello size={12} /><ITText className="font-bold text-[11px]">{tt("admintasks.viewBoard")}</ITText></ITFlex>
          </ITButton>
          <ITButton variant="outlined" onClick={() => setReloadKey((key) => key + 1)}>
            <ITFlex align="center" gap={1}><FaSync size={11} /><ITText className="font-bold text-[11px]">{tt("admintasks.refresh")}</ITText></ITFlex>
          </ITButton>
        </ITFlex>
      }
    >
      <ITFlex justify="end" align="center" gap={2} className="mb-3">
        {loading && <ITText className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{tt("admintasks.loading")}</ITText>}
