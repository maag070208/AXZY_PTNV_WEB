...
export default function TicketDetailPage() {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    if (updateTicketThunk.fulfilled.match(action)) {
      ...
      setToast(tt("detail.statusChanged", { status: STATUS_LABELS[newStatus] ?? newStatus }));
    }
  };
  ...
  const handleCategoryChange = async (newCategory: string) => {
    ...
    if (updateTicketThunk.fulfilled.match(action)) {
      refresh();
      setToastType("success");
      setToast(tt("detail.categoryChanged", { category: CATEGORY_LABELS[newCategory] ?? newCategory }));
    }
  };
  ...
  const handleDepartmentChange = async (newDepartmentId: string) => {
    ...
    if (updateTicketThunk.fulfilled.match(action)) {
      ...
      setToast(tt("detail.departmentChanged", { department: dept?.name ?? "Sin asignar" }));
    }
  };
  ...
  const handleResponsibleChange = async (userId: string) => {
    ...
    if (updateTicketThunk.fulfilled.match(action)) {
      refresh();
      setToastType("success");
      setToast(userId ? tt("detail.responsibleAssigned") : tt("detail.responsibleUnassigned"));
    }
  };
      ...
      return (
        <ITFlex justify="between" align="center" className="w-full rounded-2xl border border-slate-100 bg-slate-50/70 px-4 py-3">
          <ITFlex align="center" gap={2}>
            ...
            <ITText className="text-[11px] font-black uppercase tracking-widest text-slate-500">{tt("detail.tasksGraph")}</ITText>
                ...
                <div className={`flex-1 pb-3 min-w-0 rounded-xl border-l-4 ${meta.bar} bg-white border border-slate-200 shadow-sm p-3`}>
                  <ITFlex justify="between" align="center" gap={2} className="mb-2">
                    ...
                    <ITFlex align="center" gap={2} className="shrink-0">
                      ...
                      >
                        <ITText className="text-[10px] font-bold">{assignmentOpen ? tt("detail.hide") : tt("detail.open")}</ITText>
                      </ITButton>
                    </ITFlex>
                  </ITFlex>
            ...
            <div className="flex-1 min-w-0 rounded-2xl border border-slate-100 bg-slate-50/70 p-3">
              <ITFlex justify="between" align="center" gap={2}>
                <ITFlex align="center" gap={2}>
                  ...
                  <ITText className="text-[11px] font-black uppercase tracking-widest text-slate-600">{tt("detail.newTask")}</ITText>
                </ITFlex>
                <ITButton variant="outlined" color="secondary" size="small" onClick={() => setNewTaskOpen((open) => !open)}>
                  <ITText className="text-[10px] font-bold">{newTaskOpen ? tt("detail.hide") : tt("detail.openForm")}</ITText>
                </ITButton>
              </ITFlex>
                ...
                <ITGrid item xs={12}>
                  <ITFlex justify="end">
                    ...
                    >
                      <ITFlex align="center" gap={1}>
                        <FaPlus size={12} />
                        <ITText className="font-bold text-[11px]">{tt("detail.assign")}</ITText>
                      </ITFlex>
                    </ITButton>
                  </ITFlex>
                </ITGrid>
          ...
          >
            <ITFlex align="center" gap={1}>
              ...
              <ITText className="font-bold text-[11px]">
                {downloadingPDF ? tt("detail.pdfGenerating") : tt("detail.pdf")}
              </ITText>
            </ITFlex>
           </ITButton>
